...
Layer 9 is different in kind. **Utilisation is a property of whose demand you serve, not of how well you serve it.** A provider's tenants peak at different hours for unrelated reasons; an enterprise's applications peak together, because they serve the same business on the same working day. That is not an engineering deficit and no amount of engineering closes it — a well-run single-tenant fleet still loses this layer to an indifferently-run multi-tenant one.

It can be *partially* recovered by consolidating applications onto one platform, multiplexing models, and shifting schedulable work into troughs. It cannot be fully recovered, because what is missing is demand that is uncorrelated with your own, and that is not something you can procure. Any analysis treating build-versus-rent as a purely engineering comparison has assumed exactly this away.
...
### 12.1 The pattern has run before, and there is a specific instance

Google published PageRank in 1998. The algorithm was the breakthrough and it is what the company is remembered for.

It is not what made the company durable. Serving that algorithm to the world required years of systems work that was never especially visible from outside — scheduling computation across enormous fleets, placing and replacing jobs as machines failed, keeping utilisation high enough that the economics worked at all. That work produced Borg, and Borg's lineage produced Kubernetes, which is now the substrate a large fraction of the industry runs on.

So the algorithm was the intellectual property. **The serving system was the moat.** And the serving system, not the algorithm, is what the rest of the industry eventually had to be given in order to catch up.

The same shape recurs whenever a valuable input becomes cheap. Compute became an API call around 2008, and the confident prediction — widely made, and wrong — was that operational work would shrink because the hard part had been outsourced. What happened instead is that the work moved. Provisioning collapsed and everything downstream of it expanded: deployment, configuration, monitoring, incident response, capacity planning. Within a decade that body of work had a name, then two, along with job titles that had not previously existed, an error-budget vocabulary that let engineers and executives negotiate in the same units, and a set of companies built entirely on operating other people's software reliably.

None of that existed because compute got expensive. It existed because **compute got cheap enough that the difficulty moved somewhere with no name.**
